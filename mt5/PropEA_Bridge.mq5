...
{
   int pos = StringFind(InpApiUrl, "/trade_signal");
   if(pos < 0)
      return "http://127.0.0.1:8000/health";
   return StringSubstr(InpApiUrl, 0, pos) + "/health";
}

//+------------------------------------------------------------------+
string WebRequestStatusHint(const int status, const int err_code)
{
   if(status == 1001)
      return "接続中断(1001): Python Bridge 停止・再起動・FW を確認";
   if(status == 1003)
      return "WebRequest競合(1003): 同時HTTPまたはURL不一致 — 他EA/前リクエスト完了待ち。未許可なら MT5→オプション→EA→WebRequest に http://127.0.0.1:8000";
   if(status == 1004)
      return "タイムアウト(1004): M1/SMRS 履歴7500本で応答遅延 — Bridge CPU/再起動";
   if(status == -1 && err_code == 4014)
      return "WebRequest不可(4014): 許可URL未設定または HTTPS 証明書問題";
   if(status == -1)
...
{
   static bool g_pipeline_in_flight = false;
   if(g_pipeline_in_flight)
      return;
   ...
   // Bridge 連続失敗時は照会間隔を空けて負荷・ログ洪水を抑制（EA クラッシュ防止）
   if(g_http_fail_streak > 0)
   {
      int backoff_sec = MathMin(300, InpTimerSeconds * (1 + g_http_fail_streak));
      if(TimeCurrent() - g_last_request < backoff_sec)
      {
         g_pipeline_in_flight = false;
         return;
      }
...
void OnDeinit(const int reason)
{
   EventKillTimer();
   DbbsExit_Deinit();
   string why = "code=" + IntegerToString(reason);
   switch(reason)
   {
      case REASON_REMOVE:      why = "removed by user"; break;
      case REASON_CHARTCLOSE:  why = "chart closed"; break;
      case REASON_RECOMPILE:   why = "recompiled"; break;
      case REASON_INITFAILED:  why = "OnInit failed"; break;
